...
&= \sum_s Pr[S_t=s] \left[\sum_a q_{\pi(\theta)}(s,a)\nabla\pi(a \mid s;\theta) + \sum_{s'}Pr(S_{t+1}=s' \mid S_t=s;\theta) \gamma \nabla v_{\pi(\theta)}(s')  \right] \\
&= \sum_s Pr[S_t=s] \sum_a q_{\pi(\theta)}(s,a)\nabla\pi(a \mid s;\theta) + \sum_s Pr[S_t=s] \sum_{s'}Pr(S_{t+1}=s' \mid S_t=s;\theta) \gamma \nabla v_{\pi(\theta)}(s') \\
&= \sum_s Pr[S_t=s] \sum_a q_{\pi(\theta)}(s,a)\nabla\pi(a \mid s;\theta) + \gamma \sum_s Pr[S_{t+1}=s';\theta] \nabla v_{\pi(\theta)}(s') \\
...
解决办法是在训练时，将 $\Psi_t$ 作为样本权重， $\pi(A_t \mid S_t;\theta)$ 作为输出结果；即 $\Psi_t$ 大的样本，其权重也大，更新幅度越大，这样才符合算法的思想。另外，每个回合后进行随机梯度下降应使用整个回合作为一批样本，对应函数为 `train_on_batch` ，修改后的代码如下：
